...
/*   Updated: 2025/06/23 08:01:08 by mlaussel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	e->s.fov = PI * 0.33;
	e->s.rh = 2 * tan(e->s.fov * 0.5) / win_scale;
	e->s.rv = 2 * tan(e->s.fov * 0.5) / win_scale;
}
...
void	ft_director_vector(t_exec *e, int i, int j, int win_scale)
{
	...
	ray->dir_x = (i - win_scale * 0.5) * e->s.rh;
	ray->dir_y = -1.0;
	ray->dir_z = (win_scale * 0.5 - j) * e->s.rv;
