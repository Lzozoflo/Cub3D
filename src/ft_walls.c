...
/*   Updated: 2025/06/04 15:21:42 by mlaussel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_walls(t_data *d)
{
	...
	// 6- Boucle DDA, avancer case par case jusqu'à toucher un mur
	w.hit = 0;
	while (w.hit == 0)
	{
		if (w.side_x < w.side_y)
		{
			w.side_x += w.dist_x;
			w.pos_x += w.step_x;
			w.side = 0; // mur vertical
		}
		else
		{
			w.side_y += w.dist_y;
			w.pos_y += w.step_y;
			w.side = 1; // mur horizontal (corrigé ici, tu avais 0 au lieu de 1)
		}
